...
image = Image.open('Images//zinghr.PNG')
st.image(image, use_column_width=True)

st.title("Resume Matcher by Analytics Team")
...
# Checking for Multiple Job Descriptions
# If more than one Job Descriptions are available, it asks user to select one as well.
if len(Resumes['Name']) <= 1:
    st.write(
        "There is only 1 Resume Description present. It will be used to create scores.")
else:
    st.write("There are ", len(Resumes['Name']),
             "Resume Descriptions available. Please select one.")


# Asking to Print the Job Description Names
if len(Resumes['Name']) > 1:
    option_yn_resume = st.selectbox(
        "Show the Resume Description Names?", options=['YES', 'NO'])
    if option_yn_resume == 'YES':
        index_resume = [a for a in range(len(Resumes['Name']))]
        fig = go.Figure(data=[go.Table(header=dict(values=["Resume No.", "Resume Desc. Name"], line_color='darkslategray',
                                                   fill_color='lightskyblue'),
                                       cells=dict(values=[index_resume, Resumes['Name']], line_color='darkslategray',
                                                  fill_color='cyan'))
                              ])
        fig.update_layout(width=700, height=400)
        st.write(fig)
...
# Asking to chose the Resume Description
index_resume = st.slider("Which Resume to select ? : ", 0,
                  len(Resumes['Name'])-1, 1)


option_yn_resume = st.selectbox("Show the Resume Description ?", options=['YES', 'NO'])
if option_yn == 'YES':
    st.markdown("---")
    st.markdown("### Resume Description :")
    fig = go.Figure(data=[go.Table(
        header=dict(values=["Resume Description"],
                    fill_color='#f0a500',
                    align='center', font=dict(color='white', size=16)),
        cells=dict(values=[Resumes['Context'][index_resume]],
                   fill_color='#f4f4f4',
                   align='left'))])
